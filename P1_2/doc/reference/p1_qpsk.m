function [ber, ber_th] = p1_2_qpsk(EbN0dB, isGray)
...
modulation_values = pskmod(0:M-1, M);
...
if isGray;
    simb_sent = pskmod(simb, M, 0, 'GRAY');
else
    simb_sent = pskmod(simb, M);
end
...
%%  Demodulamos
if isGray;
    simb_recv = pskdemod(recv, M, 0, 'GRAY');
else
    simb_recv = pskdemod(recv, M);
end
