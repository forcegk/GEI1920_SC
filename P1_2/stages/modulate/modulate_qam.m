...
function [simb_stream_out, Eb] = modulate_qam(bit_stream_in, M, is_gray)
    %% Calculamos k   
    k = log2(M);
    ...
    simb_stream_out = qammod(0:(M)-1, (M));
    ...
    if is_gray;
        simb_stream_out = qammod(simb, M, 0, 'GRAY');
    else
        simb_stream_out = qammod(simb, M);
    end
